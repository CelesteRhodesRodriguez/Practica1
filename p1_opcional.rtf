...
"""\
\pard\tx566\tx1133\tx1700\tx2267\tx2834\tx3401\tx3968\tx4535\tx5102\tx5669\tx6236\tx6803\pardirnatural\partightenfactor0
\cf0 Pr\'e1ctica 1: Programaci\'f3n concurrente (parte opcional). De: Celeste Rhodes Rodr\'edguez.\
\pard\tx566\tx1133\tx1700\tx2267\tx2834\tx3401\tx3968\tx4535\tx5102\tx5669\tx6236\tx6803\pardirnatural\partightenfactor0
\cf0 """\
\
...
def main():\
    ...
    index1 = [Value('i', 0) for i in range(NPROD)] #index1 para dentro de cada buffer indicar cual es el siguiente hueco vac\'edo/cual es el ult elem\
    consumer_list=[]\
    for i in range(NPROD): \
        for k in range(K):\
            storagelst[i][k] = -2\
...
\
    ...
    """\
  \
    non_empty = [Semaphore(0) for i in range(NPROD)] #dice si est\'e1 vac\'edo.\
    empty = [BoundedSemaphore(K) for i in range(NPROD)] #representa el numero de huecos en el buffer. -1 cuando a\'f1ades(wait), +1 cuando quitas(signal), 0 completo(no te dejo pasar)\
